...
/***/ function(module, exports, __webpack_require__) {
	...
	var _bingle = __webpack_require__(300);

	var _bingle2 = _interopRequireDefault(_bingle);

	var _registerName = __webpack_require__(301);

	var _registerName2 = _interopRequireDefault(_registerName);

	var _socketHandler = __webpack_require__(303);

	var _socketHandler2 = _interopRequireDefault(_socketHandler);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	new _bingle2.default();
	new _registerName2.default();
	new _socketHandler2.default();
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _requests = __webpack_require__(302);
	...
	var Bingle = function () {
	  function Bingle() {
	    ...
	    _classCallCheck(this, Bingle);

	    if (!Bingle.initialized) {
	      console.log('initialized bingle...');
	      var register = new _registerName2.default();
	      ...
	      [].forEach.call(document.getElementsByClassName('bingo-tile'), function (tile) {
	        tile.addEventListener('click', _this.selectTile.bind(_this, tile), false);
	        tile.addEventListener('touch', _this.selectTile.bind(_this, tile), false);
	      });
	  ...
	  _createClass(Bingle, [{
	    key: 'selectTile',
	    value: function selectTile(tileElem, event) {
	      var _this2 = this;

	      var toggleSelect = function toggleSelect(select) {
	        if (select) {
	      ...
	      var coordinates = tileElem.getAttribute('data-coordinates').split('-');
	      var selected = tileElem.classList.contains('bingo-tile--selected');

	      toggleSelect(!selected);

	      (0, _requests.doPost)('/api/player/select-tile', { row: coordinates[1], col: coordinates[0] }).then(function (tile) {
	    ...
	    key: 'updateLuckyCrowdStats',
	    value: function updateLuckyCrowdStats() {
	      var statElem = document.getElementById('pole-position');
	      (0, _requests.doGet)('/api/crowd/stats').then(function (stats) {
	        statElem.innerHTML = stats.map(function (stat, index) {
	          var present = index + 1 + '. ' + stat.name + ' ';
	          present = present.replace(/\s+/g, '&nbsp;');
	          return present;
	        }).join(' ');
	      }).catch(function (e) {
	        return console.log('Could not get players data: ', e);
	      });
	    }
	  }, {
	    key: 'updateTheCrowd',
	    value: function updateTheCrowd() {
	      var statElem = document.getElementById('buzzing-crowd');
	      var numberToSymbol = function numberToSymbol(number) {
	        var symbol = number;
	      ...
	      (0, _requests.doGet)('/api/crowd/all').then(function (crowd) {
	        statElem.innerHTML = crowd.map(function (player) {
	          return player.name + '&thinsp;' + '[' + player.numSelected + '&thinsp;' + numberToSymbol(player.numBingos) + ']';
	        }).join(' vs ') || '&nbsp;';
	      }).catch(function (e) {
	        return console.log('Could not get the crowd: ', e);
	      });
	    }
	  }, {
	    ...
	    value: function updatePlayerStats() {
	      ...
	      (0, _requests.doGet)('/api/player/stats').then(function (stats) {
	    ...
	    key: 'randomiseBingoTileColors',
	    value: function randomiseBingoTileColors() {
	      ...
	      bingoTiles.forEach(function (tile) {
	    ...
	    value: function leaveGame() {
	      (0, _requests.doPost)('/api/player/remove').then(function () {
	        document.location.reload(true);
	      }).catch(function (e) {
	        return console.log('Could not remove player: ', e);
	      });
	    }
	  }]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var RegisterName = function () {
	  function RegisterName() {
	    _classCallCheck(this, RegisterName);
	  ...
	  _createClass(RegisterName, [{
	    ...
	    value: function registerName(e) {
	      ...
	      if (!name || !name.match(/^[A-Za-z0-9_ \-!?[\]()]{3,}$/)) {
	        ...
	        (0, _requests.doPost)('/login', { name: name }).then(function (data) {
	          document.location.reload(true);
	        }).catch(function (data) {
	          document.getElementById('register-submit').removeAttribute('disabled');
	          _this.displayMessage(data.error || 'Ooops...something is wrong...');
	    ...
	    key: 'toggleRegistred',
	    value: function toggleRegistred(status) {
	    ...
	    value: function checkSession() {
	      ...
	      (0, _requests.doGet)('/login', {}).catch(function (e) {
	        _this2.toggleRegistred(false);
	        console.log(e);
	      });
	    }
	  }]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _notifications = __webpack_require__(304);
	...
	var SocketHandler = function SocketHandler() {
	  ...
	  if (!SocketHandler.initialized) {
	    (function () {
	      var bingle = new _bingle2.default();
	      var socket = io();
	      ...
	      socket.on('player_selected_tile', function (status) {
	        ...
	        if (status.new_bingo && !status.name.match(window.buzbingle_name)) {
	          (0, _notifications.notifyPlayerBingo)(status.name, status.num_bingos);
	        }
	      });
...
/***/ function(module, exports) {
	...
	exports.notifyPlayerBingo = notifyPlayerBingo;
	function notifyPlayerBingo(name, numBingos) {
